...
export class CycleService {
  private apiUrl = 'http://localhost:5085/api/cycles'; // Update with actual backend URL
      ...
      map((cycles) => 
        cycles.map(cycle => ({
          ...
          imageBase64: cycle.imageBase64 
            ...
            : '/assets/cycles/mountain.jpg' // Default image
        }))
      ),
      catchError(error => {
  ...
  getCycleById(cycleId: string): Observable<Cycle> {
    // return this.http.get<Cycle>(`${this.apiUrl}/${cycleId}`);
